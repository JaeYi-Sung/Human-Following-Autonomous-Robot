...
class Preprocessdata:
    ...
    def print_label(self, image, gesture_joints, gesture_type):
        ...
        fontScale = 2
        no_frames = 4
        print(self.prev_queue)

        if self.prev_queue == [1]* self.num_frames: # fist
            self.text = gesture_type[0]
        elif self.prev_queue == [2]* self.num_frames: # pointing
            self.text = gesture_type[1]
        elif self.prev_queue == [3]* self.num_frames: # palm
            self.text = gesture_type[2]
        elif self.prev_queue == [4]* self.num_frames: # peace
            self.text = gesture_type[3]
        elif self.prev_queue == [5]* self.num_frames: # ok
            self.text = gesture_type[4]
        else: # no hand
            self.text = gesture_type[5]
#        image = cv2.putText(image, self.text, org, font,  
#                       fontScale, color, thickness, cv2.LINE_AA) 
        cv2.putText(image, self.text, org, font, fontScale, color, thickness, cv2.LINE_AA)
        print(self.text)
        return self.text